...
const getWithInPercent = (partial: number, total: number) => (partial / total) * 100;

const BarChart: FC<BarChartProps> = ({data, colors = defaultColors}) => {

    const totalAmount = useMemo(() => data.reduce((acc, {value}) => acc + value, 0), []);
    if(data.length < colors.length){
        throw new Error('<BarChart />: data list cannot be longer than color list');
    }
    return (
        <Container>
            {data.map(({value}, index) => <ChartItem $color={colors[index]} $width={`${getWithInPercent(value, totalAmount)}%`} />)}
        </Container>
    );
}
